...
class WatchlistTestCase(unittest.TestCase):
    ...
    def test_admin_command(self):
        ...
        self.assertEqual(User.query.first().name, 'grey')
        self.assertTrue(User.query.first().valid_password('123'))

    # 测试更新管理员账户
    def test_admin_command_update(self):
        # 使用 args 参数给出完整的命令参数列表
        result = self.runner.invoke(args=['admin', '--username', 'peter', '--password', '456'])
        self.assertIn('正在更新用户...', result.output)
        self.assertIn('Done.', result.output)
        self.assertEqual(User.query.count(), 1)
        self.assertEqual(User.query.first().name, 'peter')
        self.assertTrue(User.query.first().valid_password('456'))
